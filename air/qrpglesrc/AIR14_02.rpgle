     ...
     F*   CRTRPGMOD MODULE(AIRLIB/AIR14_02) SRCFILE(AIRLIB/AIRSRC) +
     F*             DBGVIEW(*ALL) INDENT('.')
     F*
     F*   (2. CREATE THE PROGRAM)
     F*   CRTPGM PGM(AIRLIB/AIR14_02)
     F*     BNDSRVPGM(SVAIRJAVA SVAIREMAIL) ACTGRP(AIR14_02)
     D********************************************************************
     D                SDS
     D  QPGM                   1     10
     D  MSGTXT                91    170
     ...
     C/EJECT
      /free
         ...
         AirEmail_setSubject(airMessage: 'AS400 HTML With Images Email');
         recipients = *BLANKS;
         ...
         htmlString = '<h1>HTML</h1>'
                    + '<img src="'
                    + 'http://www.2wolvesout.com/graphics/mantis.png">'
                    + ' with an Image.';
