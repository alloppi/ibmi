      ...
      *
      * Function:
      *  Create BOC_KEYF file for storing encryption key info
      *  Create keystore file, BOCKEYFILE.
      *  Create a KEK in BOCKEYFILE with label BOCDTAKEK.
      *  Generate a key encrypted under BOCDTAKEK and store in BOC_KEYF file
      *
      * Refer to the i5/OSÂ® Information Center for a full
      * description of this scenario.
      *
      * Use the following command to compile this program:
      * CRTRPGMOD MODULE(ALAN/BOC_SETUP) SRCFILE(ALAN/QRPGLESRC)
      *
     H dftactgrp(*no) bnddir('QC2LE')

     FBOC_KEYF  uf a e             disk    usropn
     ...
     D algD                           1    const                                Algorithm description
     D algFormat                      8    const                                Algorithm description format name
     D AESctx                         8                                         Algorithm context token
     D errCod                         1                                         Error code
      ...
      * Delete key store BOCKEYSTR
     C                   callp     system('DLTF FILE(ALAN/BOCKEYSTR)')

      * Create file BOC_KEYF File
     C*                  callp     system('CRTPF FILE(ALAN/BOC_KEYF) SRCFILE(EN-
     C*                            CRYPT) AUT(*EXCLUDE)')

      * Create keystore file, BOCKEYSTR, and generate a key record
      * with label BOCDTAKEK.
     C                   eval      QC3D040000 = *loval
     C                   eval      QC3KS00 = 'BOCKEYSTR ALAN'
      ...
      * Generate AES key record BOCDTAKEK
     C                   eval      keyType = 22                                 AES
     C                   eval      keySize = 16                                 Key Size for AES must be 16, 24, 32
     C                   eval      QC3RL = 'BOCDTAKEK'
      ...
      * Create a key context for BOCDTAKEK
     C                   eval      keyLen  = %size(QC3D040000)
     C                   eval      keyForm = '0'
     C                   callp     CrtKeyCtx( QC3D040000 :keyLen  :'4'
     C                                       :keyType    :keyForm :*OMIT
     C                                       :*OMIT      :KEKctx  :QUSEC)

      * Create an AES algorithm context BOCDTAKEK
      ...
      * Generate a file key encrypted under BOCDTAKEK
     C                   callp     GenSymKey( keyType     :keySize
     C                                       :'0'         :'1'
     C                                       :KEKctx      :AESctx
     C                                       :'0'         :*OMIT
     C                                       :KEY         :keySize
     ...
     C                   open(e)   BOC_KEYF
     C                   if        %error = '1'
     C                   callp     Print('Open of BOC_KEYF file failed')
     C                   close     BOC_KEYF
     C                   endif
     ...
     C                               encrypted record to BOC_KEYF file')
     C                   endif
