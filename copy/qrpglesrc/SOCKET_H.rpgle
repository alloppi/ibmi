      ...
      *  (or whatever the appropriate source lib/file
      *
     ...
     ** fcntl() flags
     ...
     D    sa_data                    14a
      /endif
      **************************************************************
      * Socket Storage: Same as preceding structure
      *                 enough for any address
      **************************************************************
      /if not defined(XOPEN_SOURCE_520)
     D sockaddr_storage...
     ...
     **
     ...
     D   sun_path                   126a
      /endif
     **
     ** Host Database Entry (for DNS lookups
     **
     ...
     **
     ** Service Database Entry (which service = which port
     **
     **            struct servent {
     **              char   *s_name;
     **              char   **s_aliases;
     ...
     **     in RPG
     **     with mult/div/mvr and data structures
     **     be usable...
     **
     ...
     **
     ** Time Value Structure (for the select() function
     **
     **      struct timeval {
     **         long  tv_sec;                  /* seconds       */
     ...
     **      socket requests on a specific network adapter
     **      assign a specific port to your socket.
     **    For example
     **      bind to port 23
     **      telnets to listen on.
     **    If we bind to an address of 0
     ...
     **    the destination address should be set to *NULL
     **    should be zero.
     **
     ** --------------------------------------------------------------------
      /if not defined(XOPEN_SOURCE_520)
     ...
     **
     **
     **   The recvfrom() function receives data through a connected
     ...
     **
     **  Note: Due to conflicts with IFSIO_H
     ...
     **    Difference between this and close() is that with close
     **    actually delete the descriptor
     ...
     **   prevents your job from "blocking" on one socket read
     ...
     **
     **   ***** To help with managing the descriptor sets
     **   ***** created FD_SET
     ...
     **                 If a wait time of zero is given
     **                 return immediately.
     ...
     **   that it needs to take the descriptor
     **
     ...
     **
     **   return value is the socket descriptor taken
     ** --------------------------------------------------------------------
     ...
     **         };
     **
     **     An IP address (32-bit integer formnat) goes in
     **     hostent structure pops out.   Really
     **     havent already learned to hate the hostent structure
     **
     **   Note:  The strings & arrays used in C are often variable length
     ...
     **              it returns x'FFFFFFFF' on error.  However
     ...
     **
     **    Converts from 32-bit to dotted decimal
     ...
     **    msg_struct = (input) message to send (see msghdr
     ...
     **
     **     sock_desc = (input) socket descriptor to use
     **    msg_struct = (input) message to receive (see msghdr
     **         flags = (input) flags (like any send/recv API)
     **
     ...
     ** --------------------------------------------------------------------
      ...
      * NI_NUMERICHOST = only return numeric address
      * NI_NAMEREQD    = if DNS lookup of name fails
      *                    (otherwise
      * NI_NUMERICSERV = always return port number
      * NI_DGRAM       = return UDP services (by default
      * --------------------------------------------------------------------
