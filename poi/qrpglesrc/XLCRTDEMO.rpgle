      ...
      *>       CRTBNDRPG PGM(XLCRTDEMO) SRCFILE(QRPGLESRC) DBGVIEW(*LIST)
      *
      *
     H DFTACTGRP(*NO)
     ...
     D XLCRTDEMO       PR                  ExtPgm('XLCRTDEMO')
     ...
     D Dates           s                   like(SSCellStyle)
      ...
      /free
        ...
        if (xssf);
           SS_save(book: IFSDIR + '/xldemo3.6.xlsx');
        else;
     ...
     D   sheet                             like(ssSheet)
      /free
        ...
        ss_merge(sheet: 0: 0: 0: 3);
        ss_merge(sheet: 1: 0: 1: 3);
        ss_merge(sheet: 2: 0: 2: 3);
     ...
     D row             s                   like(SSRow)
      /free
         ...
         row = SSSheet_createRow(sheet: rowcount);
         ss_text( row: 0 : 'Big Big Corporation': LgHeading);

         rowcount += 1;
         row = SSSheet_createRow(sheet: rowcount);
         ss_text( row: 0 : 'Weekly Sales Figures': SmHeading);

         rowcount += 1;
         row = SSSheet_createRow(sheet: rowcount);
         ss_text( row: 0 : 'for the month of ' + monthname: SmHeading);
